...
TradingMethod.prototype.advice = function() {
  ...
  var diff = _.last(this.ema.diff).toFixed(3),
      open = _.last(this.candles.open).toFixed(8),
      close = _.last(this.candles.close).toFixed(8),
      high = _.last(this.candles.high).toFixed(8),
      low = _.last(this.candles.low).toFixed(8);
  ...
  var price = price = ( open + close + high + low ) / 4;
  //price = price.toFixed(8);

  if(config.normal.exchange !== 'cexio')
    price = price.toFixed(3);

  var message = '@ avg (open, close, high, low) ['
    + open + ', '
    + close + ', '
    + high + ', '
    + low + ']' + price + ' (' + diff + ')';
